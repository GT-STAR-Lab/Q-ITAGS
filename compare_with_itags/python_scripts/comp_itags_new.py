...
def main():
    ...
    print("Number of data points:", len(allocation_quality_base))
    ...
    if (len(makespan_base)) > 1:
        ...
        cat_make = np.equal(np.subtract(makespan_base, makespan_itags), 0) * 1
        data = pd.DataFrame({"X Value": makespan_base, "Y Value": makespan_itags, "Category": cat_make})
        groups = data.groupby("Category")
        # colors = [(1, 0, 0, 0.8), (0, 0.8, 0, 1)]
        colors = [(0, 0.8, 0, 0.5), (0.4, 0.4, 0.4, 0.8)]
        color_map = []
        for i in cat_make:
            color_map.append(colors[i])

        eq_idx = np.where(np.equal(np.subtract(makespan_base, makespan_itags), 0) * 1 == 1)[0]
        lt_idx = np.where(np.equal(np.subtract(makespan_base, makespan_itags), 0) * 1 == 0)[0]



        # cat_make = np.subtract(makespan_base, makespan_itags).astype(int)
        # cmap = LinearSegmentedColormap.from_list("", ["grey",  "green"])  # plt.cm.get_cmap('RdYlGn')
        ...
        ax[1].plot(xy_line, '#bbbbbb', ls="--")

        for name, group in groups:
            ax[1].scatter(group["Y Value"], group["X Value"], label='Q-ITAGS $\equiv$ ITAGS' if name == 1 else 'Q-ITAGS $\succ$ ITAGS',
                          color=colors[name])
    ...
    if (len(allocation_quality_base)) > 1:
        ...
        print(cat_make)
        cat_make = np.equal(np.subtract(allocation_quality_base, allocation_quality_itags), 0) * 1
    ...
    #         color_map.append(colors[i])
    #
    #     ax[2].scatter(compute_times_itags, compute_times_base, c=color_map)
    #     ax[2].set_xlabel('ITAGS')
    #     ax[2].set_title(f'Total Comp. Time')
    #     ax[2].set_ylabel('Q-ITAGS')
    #     ax[2].set_xlim(0, 500)
    #     ax[2].set_ylim(0, 500)
    #
    #     xy_line = np.arange(0, 500)
    #     ax[2].plot(xy_line, '#bbbbbb', ls="--")

    fig.tight_layout()


    # plt.show()
    plt.savefig('images/comp_updated.pdf')
