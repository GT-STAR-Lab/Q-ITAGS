...
{
    /*!
     * \brief Checks if a search node reaches the specified vertex
     *
    ...
    {
        using __Vertex = typename UndirectedGraphSearchNodeDeriv::Vertex;

       public:
        //! Constructor
        explicit UndirectedGraphGoalCheck(const std::shared_ptr<__Vertex>& goal)
            : m_goal(goal)
        {}
       ...
       protected:
        std::shared_ptr<__Vertex> m_goal;
    };
}  // namespace grstapse