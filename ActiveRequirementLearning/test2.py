...
class SinExpert(QE.QueryableExpert):
    def get_query_label(self, features):
        ...
        # linear = [0, 2, 2, 0, 2]
        # linear = [0, 2, 0, 0, 2]
        return np.sum(np.multiply(np.array(linear), features))
...
class sin_dataset(ARLD.ActiveRequirementLearningDataset):
    def __init__(self, size):
            ...
            self.X = []
            for i in range(size):
                self.X.append(np.random.random_integers(4, size=5).tolist())
            self.y = []
            for i in self.X:
                self.y.append(int(np.sum(np.multiply(np.array(linear), i))))
            I = 0
...
def show_performance(arl, dataset, n_initial, n_queries):
    ...
    stats =arl.train_actively(dataset)
    print(stats["r2e"])
