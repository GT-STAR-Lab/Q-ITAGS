...
namespace grstapse
{
    ...
    nlohmann::json TemporalGridCellNode::serializeToJson(
        const std::shared_ptr<const ProblemInputs>& problem_inputs) const
    {
        nlohmann::json j;
        j.push_back({{constants::k_time, m_time}, {constants::k_x, m_x}, {constants::k_y, m_y}});
        for(std::shared_ptr<const TemporalGridCellNode> node = this->parent(); node != nullptr; node = node->parent())
        {
            j.push_back({{constants::k_time, node->m_time}, {constants::k_x, node->m_x}, {constants::k_y, node->m_y}});
        }
