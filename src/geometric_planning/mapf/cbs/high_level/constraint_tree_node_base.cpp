...
namespace grstapse
{
    ...
    nlohmann::json ConstraintTreeNodeBase::serializeToJson(
        const std::shared_ptr<const ProblemInputs>& problem_inputs) const
    {
        nlohmann::json j;
        for(unsigned int i = 0; i < m_num_robots; ++i)
        {
            j.push_back(lowLevelSolution(i).back()->serializeToJson(problem_inputs));
        }
        return j;
    }
}  // namespace grstapse