...
{
    TEST(CompleteEuclideanGraphMotionPlanner, Simple)
    {
        ...
        std::ifstream fin(std::string(s_data_dir) +
                          std::string("/geometric_planning/environments/polypixel_complete.json"));
        nlohmann::json j;
        fin >> j;
        auto environment = j.get<std::shared_ptr<EuclideanGraphEnvironment>>();

        auto motion_planner = std::make_shared<CompleteEuclideanGraphMotionPlanner>(parameters, environment);
        ...
        auto result = motion_planner->query(nullptr, ic, gc);
        auto path   = std::dynamic_pointer_cast<const EuclideanGraphMotionPlannerQueryResultBase>(result)->path();
        ASSERT_EQ(path.size(), 2);
    }

    TEST(CompleteEuclideanGraphMotionPlanner, Load)
    {
        std::ifstream fin(std::string(s_data_dir) +
                          std::string("/problem_inputs/grstaps/complete_euclidean_graph_motion_planners.json"));
        nlohmann::json j;
        fin >> j;

        MotionPlannerBase::init();
        auto motion_planner = std::dynamic_pointer_cast<CompleteEuclideanGraphMotionPlanner>(
