...
namespace grstapse::unittests
{
    TEST(ItagsProblemInputs, LoadTasks)
    {
        ...
        nlohmann::json j;
        {
            std::ifstream fin(std::string(s_data_dir) + std::string("/problem_inputs/itags/load_tasks.json"));
            fin >> j;
        }
        ...
        ASSERT_EQ(tasks.size(), 1);
        {
            ...
            auto space_information = std::make_shared<ompl::base::SpaceInformation>(state_space);
            {
                ...
                ASSERT_TRUE(*(tasks[0]->initialConfiguration()) == *correct_initial_configuration);
            }
            {
                auto correct_terminal_configuration = std::make_shared<Se2StateOmplConfiguration>(10.0f, 0.0f, 0.0f);
                ASSERT_TRUE(*(tasks[0]->terminalConfiguration()) == *correct_terminal_configuration);
            }
        }
    }
#ifndef NO_MILP
