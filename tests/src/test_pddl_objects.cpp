...
namespace grstapse::unittests
{
    TEST(PddlObject, empty_constants)
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/objects/empty_constants.pddl"));
    ...
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/objects/single_type_constants.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/objects/multiple_type_constants.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        std::shared_ptr<PddlTask> task = parser.pddlTask();
        task->setRequirement("typing");
