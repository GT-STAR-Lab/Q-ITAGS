...
namespace grstapse::unittests
{
    ...
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/functions/predicates/empty.pddl"));
    ...
    TEST(PddlFunction, single_predicate_no_parameters)
    {
        FileReader reader(
            ...
            std::string("/task_planning/pddl_parser/functions/predicates/single_predicate_no_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        reader.checkNext(PddlSymbol::e_open_paren);
    ...
    TEST(PddlFunction, single_predicate_single_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string(
                "/task_planning/pddl_parser/functions/predicates/single_predicate_single_type_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    TEST(PddlFunction, single_predicate_multiple_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string(
                "/task_planning/pddl_parser/functions/predicates/single_predicate_multiple_type_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    TEST(PddlFunction, multiple_predicates_multiple_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string(
                "/task_planning/pddl_parser/functions/predicates/multiple_predicates_multiple_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/functions/numeric/empty.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        reader.checkNext(PddlSymbol::e_open_paren);
    ...
    TEST(PddlFunction, single_function_no_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string("/task_planning/pddl_parser/functions/numeric/single_function_no_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        reader.checkNext(PddlSymbol::e_open_paren);
    ...
    TEST(PddlFunction, single_function_single_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string("/task_planning/pddl_parser/functions/numeric/single_function_single_type_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    TEST(PddlFunction, single_function_multiple_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string("/task_planning/pddl_parser/functions/numeric/single_function_multiple_type_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    TEST(PddlFunction, multiple_functions_multiple_type_parameters)
    {
        FileReader reader(
            std::string(s_data_dir) +
            std::string("/task_planning/pddl_parser/functions/numeric/multiple_functions_multiple_parameters.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
    ...
    {
        FileReader reader(std::string(s_data_dir) +
                          std::string("/task_planning/pddl_parser/functions/object/object_function.pddl"));
        mocks::MockPddlParser parser;
        parser.createPddlTask();
        parser.pddlTask()->setRequirement("typing");
        std::shared_ptr<PddlTask> task = parser.pddlTask();
