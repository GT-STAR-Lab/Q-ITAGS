...
namespace grstapse::unittests
{
    ...
    TEST(GrstapProblemInputs, LoadMotionPlanners)
    {
        nlohmann::json j;
        {
            std::ifstream fin(std::string(s_data_dir) +
                              std::string("/problem_inputs/grstaps/ompl_motion_planners.json"));
            fin >> j;
        }
    ...
    TEST(GrstapProblemInputs, LoadSpecies)
    {
        ...
        {
            std::ifstream fin(std::string(s_data_dir) + std::string("/problem_inputs/grstaps/species.json"));
            nlohmann::json j;
            fin >> j;
    ...
    TEST(GrstapProblemInputs, LoadRobots)
    {
...

        {
            ...
            {
                std::ifstream fin(std::string(s_data_dir) + std::string("/problem_inputs/grstaps/robots.json"));
                fin >> robots_j;
            }
...

        {
            ...
            ASSERT_EQ(*(turtlebot3_burger->initialConfiguration()), *correct_initial_configuration);
...

        {
            ...
            ASSERT_EQ(*(turtlebot3_waffle->initialConfiguration()), *correct_initial_configuration);
    ...
    TEST(GrstapProblemInputs, CreateTasks)
    {
        ...
        nlohmann::json j;
        {
            std::ifstream fin(std::string(s_data_dir) + std::string("/problem_inputs/grstaps/tasks.json"));
            fin >> j;
        }
        ...
        {
            auto correct_initial_configuration = std::make_shared<Se2StateOmplConfiguration>(0.0, 0.0, 0.0);
            ASSERT_EQ(*(task->initialConfiguration()), *correct_initial_configuration);
        }
        {
            auto correct_terminal_configuration = std::make_shared<Se2StateOmplConfiguration>(10.0, 0.0, 0.0);
            ASSERT_EQ(*(task->terminalConfiguration()), *correct_terminal_configuration);
        }
    }
    ...
    {
        nlohmann::json j;
        std::ifstream fin(std::string(s_data_dir) + std::string("/problem_inputs/grstaps/full_run.json"));
        fin >> j;
