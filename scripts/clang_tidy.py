...
def getFiles(root_directories: List[str], recursive: bool = False, extensions: List[str] = None, excludes: List[str] = None) -> List[str]:
    '''
...
def runClangTidy(file: str,
    ...
    try:
        ...
        printError(__file__, f"Command '{subprocess.list2cmdline(invocation)}' failed to start: {error}", use_color=use_color)
        sys.exit(1)
    ...
    proc.wait()
    if proc.returncode:
        printError(__file__, f"Command '{subprocess.list2cmdline(invocation)}' returned non-zero exit status: {proc.returncode}", use_color=use_color)
        for error in errors:
...
def main():
    parser = ArgumentParser(description='A wrapper script for clang-tidy to be used as a stand alone script or for continuous integration')
    parser.add_argument(
        '--exe',
        '--executable',
